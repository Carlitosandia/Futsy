...
/**
 *
 * El DAO(Data Object Access) contiene las consultas a la bd y prepara los datos
 * obtenidos para regresar la informacion en el Modelo definido anteriormente.
 */
public class UsersDAO {
    ...
    /**
     * Cada metodo definido en esta clase esta diseñada para realizar una
     * consulta a la bd. Insert Update Delete Select
     *
     */
    ...
    public void deleteUsuario(int idUsuario) {
        PreparedStatement ps = null;
        try {
            ps = conn.prepareStatement("UPDATE users SET status = 'banned', deleted_at = NOW() WHERE id = ?");
            ps.setInt(1, idUsuario);

            int delete = ps.executeUpdate();

            if (delete != 0) {
                System.out.println("Usuario baneado exitosamente.");
            } else {
